  ...
  if ('launchQueue' in window) {
    window.launchQueue.setConsumer(async (params) => {
      ...
      try {
        ...
        if (file.type === 'application/pdf' || /\.pdf$/i.test(file.name)) openFile(file);
      } catch (_) {}
    });
  }
});
...
async function openFile(file, fileHandle = null) {
  try {
    ...
    state.annotations = (await MukorobDB.get('annotations', hash))?.items || [];
...
function printOriginalPdf() {
  ...
  if (!win) { toast('Please allow pop-ups to print the PDF.'); URL.revokeObjectURL(url); return; }
...
async function persistAnnotations() {
  ...
  await MukorobDB.put('annotations', { hash: state.fileHash, items: state.annotations });
}
...
async function renderRecentList() {
  const all = (await MukorobDB.getAll('recent')).sort((a, b) => b.lastOpened - a.lastOpened);
...
async function reopenRecent(r) {
  ...
  toast('Please locate "' + r.name + '" again.');
  $('#fileInput').click();
}
...
async function saveBytesAs(bytes, suggestedName) {
  const blob = new Blob([bytes], { type: 'application/pdf' });
...
async function loginUser(id,password){
  const user=await getUser(id); if(!user || user.disabled) return false;
  const hash=await hashPassword(password,user.passwordSalt);
  if(hash!==user.passwordHash) return false;
  state.auth.user=user; state.auth.session=randomToken(32);
  localStorage.setItem(MUKOROB_SESSION_KEY,JSON.stringify({id:user.id,token:state.auth.session,createdAt:Date.now()}));
  user.lastLoginAt=Date.now(); await MukorobDB.put('users',user); return true;
}
async function restoreSession(){
  try{
    ...
    state.auth.session=s.token; state.auth.user=user; return true;
...
async function initMukorobAuth(){
  ...
  else if(await restoreSession()){ $('#authModal').hidden=true; updateAuthUI(); }
...
async function adminResetUser(id){
  ...
  const p=await makePasswordRecord(pwd); u.passwordHash=p.hash;u.passwordSalt=p.salt;u.updatedAt=Date.now();await MukorobDB.put('users',u);
  toast('Password reset. Give the temporary password to the staff member securely.');
}
async function toggleUser(id){const u=await getUser(id);if(!u)return;u.disabled=!u.disabled;await MukorobDB.put('users',u);await renderUserList();toast(u.disabled?'User disabled.':'User enabled.');}
async function deleteUser(id){if(!confirm('Delete staff user '+id+'? This cannot be undone.'))return;await MukorobDB.del('users',id);await renderUserList();toast('User deleted.');}
...
buildRecentRow=function(r){
  ...
  if(!row.querySelector('.recent-delete')){
    ...
    b.addEventListener('click',async e=>{e.preventDefault();e.stopPropagation();await MukorobDB.del('recent',r.hash);await renderRecentList();toast('Removed from Recent.');});
    row.appendChild(b);
  }
  return row;
};
...
renderPage=async function(pageNum,force=false){
  ...
  const page=await state.pdfDoc.getPage(pageNum);
  const dpr=window.devicePixelRatio||1, outputScale=Math.min(4,Math.max(2,dpr*1.25));
...
printOriginalPdf=function(){if(!requirePermission('print'))return;v05PrintOriginalPdf();};
const v05DownloadOriginalPdf=downloadOriginalPdf;
downloadOriginalPdf=function(){if(!requirePermission('download'))return;v05DownloadOriginalPdf();};
const v05SaveCurrentPdfAs=saveCurrentPdfAs;
saveCurrentPdfAs=async function(){if(!requirePermission('download'))return; if(state.pageRotations.size)return saveCurrentPdfWithRotations(state.fileName||'Mukorob-document.pdf');return v05SaveCurrentPdfAs();};

/* Permission guards around page operations. */
const v05DeletePage=deleteCurrentPage; deleteCurrentPage=async function(){if(requirePermission('edit'))return v05DeletePage();};
const v05MovePage=moveCurrentPage; moveCurrentPage=async function(){if(requirePermission('edit'))return v05MovePage();};
const v05Extract=extractPages; extractPages=async function(){if(requirePermission('edit'))return v05Extract();};
const v05Merge=mergePdfs; mergePdfs=async function(files){if(requirePermission('edit'))return v05Merge(files);};
...
function startESignature(){if(!requirePermission('signature'))return;if(!state.pdfDoc)return toast('Open a PDF first.');$('#annotToolbar').hidden=false;setTool('signature');toast('Tap or click the document where the e-signature should appear.');}
function startCompanyStamp(){if(!requirePermission('stamp'))return;if(!state.pdfDoc)return toast('Open a PDF first.');if(state.settings.companyStamp){const bin=atob(state.settings.companyStamp.split(',')[1]);const bytes=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)bytes[i]=bin.charCodeAt(i);const mime=(state.settings.companyStamp.match(/^data:([^;]+)/)||[])[1]||'image/png';placeStampImage(new File([bytes],'company-stamp.'+(mime.includes('jpeg')?'jpg':'png'),{type:mime}));}else $('#stampInput').click();}
...
exportAnnotatedPdf=async function(){
  if(!requirePermission('annotate'))return; if(!state.fileBytesForExport)return;
  if(!state.annotations.length && !state.pageRotations.size)return toast('No edits or annotations to export yet.');
  try{
    toast('Preparing edited PDF…'); const {PDFDocument,rgb}=PDFLib; const pdfDoc=await PDFDocument.load(state.fileBytesForExport); const pages=pdfDoc.getPages();
    for(let i=0;i<pages.length;i++){const r=state.pageRotations.get(i+1);if(typeof r==='number'&&r)pages[i].setRotation(PDFLib.degrees(r));}
    for(const a of state.annotations){const page=pages[a.page-1];if(!page)continue;const {width,height}=page.getSize();const col=hexToRgbFloat(a.color||'#E2A321');
      ...
      else if(a.type==='ink')for(let i=1;i<a.points.length;i++){const p0=a.points[i-1],p1=a.points[i];page.drawLine({start:{x:p0.x*width,y:height-p0.y*height},end:{x:p1.x*width,y:height-p1.y*height},thickness:2,color:rgb(col.r,col.g,col.b)});}
      else if(a.type==='signature'){const font=await pdfDoc.embedFont(PDFLib.StandardFonts.HelveticaOblique);page.drawText(a.text||'Signature',{x:a.rect.x*width,y:height-(a.rect.y+a.rect.h*.78)*height,size:Math.max(10,a.rect.h*height*.72),font,color:rgb(col.r,col.g,col.b)});}
      else if(a.type==='note'){const x=a.rect.x*width,y=height-a.rect.y*height;page.drawRectangle({x,y:y-10,width:10,height:10,color:rgb(col.r,col.g,col.b)});if(a.text)page.drawText(a.text.slice(0,90),{x:x+14,y:y-10,size:8,color:rgb(col.r,col.g,col.b)});}
      else if(a.type==='stamp'&&a.src){try{const raw=a.src.split(',')[1];const bin=atob(raw);const bytes=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)bytes[i]=bin.charCodeAt(i);let image;if(/^data:image\/png/i.test(a.src))image=await pdfDoc.embedPng(bytes);else image=await pdfDoc.embedJpg(bytes);page.drawImage(image,{x:a.rect.x*width,y:height-(a.rect.y+a.rect.h)*height,width:a.rect.w*width,height:a.rect.h*height});}catch(err){console.warn('stamp export failed',err);}}
    }
    const bytes=await pdfDoc.save();await saveBytesAs(bytes,state.fileName.replace(/\.pdf$/i,'')+' (Mukorob edited).pdf');
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  $('#btnLogin').addEventListener('click',async()=>{
    ...
    if(!ok){$('#loginError').hidden=false;return;} $('#loginError').hidden=true;$('#loginPassword').value='';$('#authModal').hidden=true;updateAuthUI();toast('Welcome, '+(state.auth.user.fullName||state.auth.user.id)+'.');
  });
  ...
  $('#btnCreateUser').addEventListener('click',async()=>{
    ...
    if(!/^[a-z0-9._-]{3,32}$/.test(id))return toast('Enter a valid user ID.');if(pwd.length<8)return toast('Temporary password must be at least 8 characters.');if(await getUser(id))return toast('That user ID already exists.');
    ...
    $('#newUserName').value='';$('#newUserFullName').value='';$('#newUserPassword').value='';await renderUserList();
  ...
  showAdminPanel=function(){oldShowAdminPanel();renderPermissionChecks();renderUserList();};
  updateAuthUI();
  initMukorobAuth();
});
