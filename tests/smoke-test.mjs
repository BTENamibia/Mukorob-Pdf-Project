...
for(const store of ['settings','recent','annotations','users','recent_scoped','annotations_scoped','drafts','audit'])assert.match(db,new RegExp(`['"]${store}['"]`));
...
assert.match(fixes,/v071RotateLeft/);assert.match(fixes,/v071RotateRight/);assert.match(fixes,/v071-annotation-control/);assert.match(fixes,/v071-delete/);assert.match(fixes,/docPointFromDisplay/);assert.match(fixes,/dashboardLogo/);assert.match(fixes,/loginBackgroundLogo/);assert.match(fixes,/loginIconLogo/);assert.match(fixes,/touchstart/);assert.match(fixes,/updateViaCache:'none'/);
